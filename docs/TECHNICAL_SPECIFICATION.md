...
**Version 1.15 — 2026-08-11 — Status: accepted, implementation-ready.** (Changelog at the bottom; v1.0 was 2026-07-17.)
...
The contract is asserted by a test rather than by this paragraph — see §16.4 — and what that test cannot assert is recorded there too.
...
**What it deliberately cannot assert, and where that is recorded.** That a control is reachable inside a 375px viewport. Playwright can do it — the harness drives a real browser and `BoundingBoxAsync` answers exactly that question — and it is the assertion F-59 would have failed. It is not here because the fifteen §16.3 scenarios share one browser context, so giving one of them a second viewport is either a second context per run or a resize every later scenario inherits, and getting that wrong produces a suite that fails on a correct tree (F-41). Recorded as the first open item of Stage 6 in `docs/MENU_AND_HANDHELD_PLAN.md` rather than left as a gap nobody wrote down.
