...
**Version 1.59 — 2026-08-30 — Status: accepted, implementation-ready.** (Changelog at the bottom; v1.0 was 2026-07-17.)
...
**The browser level reaches the surfaces `HandheldSurface` names, and no others.** It names §11.1's guest ordering surface (scenario 21), §11.2's kitchen board (scenario 6), §11.3's counter board and bill at the till (scenario 10), and §11.4's administration pages (scenario 16) — every station somebody works *from* while service is running. **The set is a list and never a count.** Slice 73 wrote *it names four* into this paragraph and, in the sentence beside it, said the set was written down rather than counted while doing both — one slice before the slice that made it six (**F-129**). A number is a second copy of the list that no gate can read, and the copy is the half that rots (**F-77**, **F-89**).

**What is outside the set is stated rather than left to be inferred**, because a surface this section governs and the barrier does not reach is unmeasured whichever the reason (**F-118**, **F-127**). Two things are outside it. **§11.5's display is a wall screen rather than a handset** — it is the one surface R§1's sentence is not about, and a 375px verdict over it would assert a width nobody views it at. **§11.6's account pages and §11.8's registration form are crossed once and declare no control of their own**: every field, every button row and every submit on them is the shared `.form-field`, `.form-actions` and `.button-*` vocabulary that four measured surfaces already carry under these same floors. That is a deferral with a stated reason rather than a decision, and the day one of those pages declares a control of its own is the day it needs a surface record here.

**A control is what a person presses to make something happen**, and that is what the barrier's selectors name: a button, a link that acts as one, a field. A link inside a sentence is body text and is measured by nothing here. That boundary is deliberate and is the reason `.counter-back a` and its kind are absent from the sets above.
...
6. Kitchen fulfills one line → guest sees fulfilled badge. **And the kitchen does all of it from a 375×667 handset**, which is Stage 1g and the last surface §11.12 governs that had never been laid out at the width R§1 says it is worked at. That is a second subject in one scenario, and it is here rather than in a scenario 22 on the reason Slices 59, 60, 61, 71 and 73 each gave: *the arrangement already exists*. A barrier over §11.2 wants a live board with work on the pass, a ticket carrying its own control, an 86 panel with something in it, and an alert that has not been cleared — which is this scenario's middle, exactly as it already stands. What it did **not** already have is a kitchen credential: `KitchenJourneys.OpenAsync` runs on the administrator's wide page in every other scenario that opens a board, so the station most of R§1's sentence is about was being worked by an administrator at a desk. This scenario now creates a kitchen account, signs it in by password on an isolated 375×667 context, completes the forced change, opens the board **before** the send so the alert arrives at a page that is watching, and **fulfils the line from there**. The administrator's board stays open and becomes the observer, which is a stronger reading of the badge claim than the one it replaces — the fulfilment and the reading are now on two contexts held by two people. **Two measurements, and the moment each is taken is the ruling.** The pass is measured **while the alert still stands**, because §10.3 renders `.kitchen-alert-badge` only while something is unseen and appending a fulfilment acknowledges it — measured after the away, that selector matches nothing and every verdict would be true of a quieter board than the one somebody works. The board is measured again **once a line is away**, because `.kitchen-recent-line`'s Undo exists only inside §11.2's fulfilment window, and a barrier that never saw it would report a board with no way to recall a dish. **Both are measured where they stand.** The unseen count and the armed state are circuit fields, so a navigation would reset the first and re-render the second in order to look at them — the same structural point scenarios 10 and 21 each make, for the same reason. **Every selector in both sets is required to match, for reach and for text**, so a renamed class fails by name; and the text floor gets its button subjects in the slice that names the surface rather than a slice later, which is **F-127**'s lesson applied on the way in.
...
| Menu Stage 1g (enhancement, not a finding) | §16.3 scenario 6's kitchen member works a 375×667 handset: the account is created, signed in and the line fulfilled from there, with the pass measured while the alert still stands and the board measured again once a line is away. The administrator's wide board becomes the observer. Stage 1 closes | R§1 · S§11.2, §11.12, §16.3 scenario 6 · `MENU_AND_HANDHELD_PLAN.md` |
| F-129 | **The count goes and the list stays.** §11.12 names its surfaces and states no number, because a number is a second copy of the list no gate can read and the slice that wrote *four* falsified it one slice later. The same paragraph now states what is **outside** the set and why, so the two halves of the sentence cannot drift apart again | S§11.12 · F-41, F-77, F-89 |
...
- **v1.59 — 2026-08-30.** Stage 1g, and the end of the handheld contract's own stage. **§16.3 scenario 6** gains the kitchen's half: a kitchen credential is created, signed in by password on an isolated 375×667 context, and the line is fulfilled **from the handset** while the administrator's wide board watches — the station R§1 is most about had been worked by an administrator at a desk in every scenario this repository has. Two measurements, at the two moments the board has its controls: the pass **while the alert still stands**, because working a line acknowledges it; the board again **once a line is away**, because Undo exists only inside the fulfilment window. Both are measured where they stand, because the unseen count and the armed state are circuit fields. **§11.12 stops counting its own surface set** — Slice 73 wrote *it names four* beside a sentence claiming the set was written down rather than counted, one slice before the slice that made it six (**F-129**) — and states instead what is outside the set and why: §11.5's display is a wall screen, and §11.6 and §11.8 declare no control of their own. No production code changed, no stylesheet changed, and no new test class exists.
