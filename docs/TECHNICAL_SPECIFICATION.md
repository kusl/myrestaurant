...
**Version 1.54 — 2026-08-27 — Status: accepted, implementation-ready.** (Changelog at the bottom; v1.0 was 2026-07-17.)
...
`tests/MyRestaurant.WebApplication.Tests/Deployment/ContainerLoggingContractTests.cs` — 2 assertions: every fixture that builds a container silences the Testcontainers logger, and silences it before it builds, over a subject set computed from the sources that construct a builder (**F-125**).
...
| F-125 | Both fixtures assign `TestcontainersSettings.Logger` before they build, inside the `try` that already turns a container failure into a skip; `ContainerLoggingContractTests` holds it over a computed subject set | §16.4 |
