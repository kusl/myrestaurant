...
**Version 1.18 — 2026-08-12 — Status: accepted, implementation-ready.** (Changelog at the bottom; v1.0 was 2026-07-17.)
...
**The contract is asserted twice, at two levels, and neither level can reach the other's claim.** Its *structure* — one breakpoint in the whole tree, one shared vocabulary, a label on every cell, the retired names gone, the area row rendered once, every custom property declared — is arithmetic on text and is asserted by a unit test. Whether a control is **on the screen** is a question about layout, so it is asserted by a browser: §16.3 scenario 16 lays a context out at 375×667, walks the administration indexes, and compares three numbers the page computes against the viewport. That second assertion is the one F-59 would have failed, and the reason it took an extra slice to arrive is itself recorded (**F-62**). Both are specified in §16.4.
...
16. Admin works §11.4's administration surfaces at 375×667 → no surface is wider than its own viewport, every row's action and every filter's submit lies inside it, every control is at least 44px tall (§11.12). Four surfaces when written, six since Stage 1b converted the two explorers; the list is the migration.
...
The **handheld layout test** (`tests/MyRestaurant.WebApplication.Tests/Components/HandheldLayoutContractTests.cs`, an ordinary unit test in the `unit` job) asserts §11.12 (**F-59**, **F-63**, **F-64**). Six assertions, and the choice of which six is the interesting part, because whether a screen is *comfortable* is a judgement no test will make. What is decidable is the structure of the rule.
...
**The set of surfaces this barrier visits is a list, and the list is the migration.** Four when it was written, the four Slice 30 restructured; six since Slice 33 added the two explorers, and Stage 1b of `docs/MENU_AND_HANDHELD_PLAN.md` adds a line per page it converts. Same arrangement as the handheld layout test's own `StillExpectedToCarryRetiredTableVocabulary` and for the same reason (F-47) — finishing is then something somebody decides rather than something nobody notices.

**A filter's own submit is a measured control**, and the membership rule it follows is *the thing an operator opened the page in order to press* rather than *a record row's action*. §11.4 makes both explorers read-only, so neither has a record action or a page-head action at all: a barrier that measured only those two selectors would visit `/administration/events` and `/administration/hidden-records` and measure nothing on either, which is the empty-set failure the count guard exists to catch (F-41). The stream checkboxes inside a filter are deliberately outside it: a checkbox is 1.35rem by declaration, so the target is the `.filter-choice` row around it, and asserting a 44px box on the input would report a finding on a correct tree. **Two of the six surfaces are measured with nothing in their list** — sittings since the barrier was written, hidden-records since Slice 33 — because populating either needs another scenario's whole arrangement, so what is untested on those two is a record card rather than the page.
