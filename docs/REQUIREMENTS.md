...
**Revision 6 — 2026-08-11.** Revision 2 folded in the owner's rulings on documentation-review findings F-01 through F-33 (see `docs/DOCUMENTATION_REVIEW.md` for the ledger and `docs/adr/` for the decision records). Every revision since has added exactly one cross-cutting principle to §8: rev 3, the running program must offer its own source (F-39); rev 4, published software must be reportable-to (F-42); rev 5, a program published for strangers to run must constrain what a browser may do with its pages (F-49); rev 6, the program must be operable on the screen §1 says it is used from (F-59). Earlier revisions are preserved in git history. The companion `docs/TECHNICAL_SPECIFICATION.md` is the normative implementation contract; where this document states intent and that document states mechanism, the mechanism governs.

*This header said "Revision 4" from 2026-08-06 until 2026-08-11, while the revision history below already carried a rev 5 entry, and cited the specification at v1.6 while it was at v1.14 — **F-58**. Both are fixed here and both are now checked by a test rather than remembered; the specification version is not restated at all, because a version of another document is a restatement joined to its subject only by somebody remembering to edit it.*
...
Administrators create and edit menu items (name, price, description, section, position, active flag); kitchen, counter, and administrators may activate/deactivate ("86") items instantly. Menu changes are their own append-only event log. Guest adds re-validate item activeness inside the send transaction; prices are captured on the line at add time, so later menu price changes never move an existing line. Deactivating an item does **not** hide it from the guest menu: it remains visible, marked "currently unavailable", and unorderable — better for a guest to see the salmon is out than to watch it mysteriously vanish. Deactivation never touches existing orders. There is no inventory or stock tracking (§9); if an item runs out, staff deactivate it. Customization notes are free text and are never validated by any rules engine — an impossible request ("eggless omelette") is handled by a human walking to the table.

**A menu has sections, and every item is in exactly one.** Drinks, breakfast, mains — headings are how a menu is read, and each item under a heading carries a **description**: a sentence explaining it, which is not decoration but the thing a guest with no waiter standing over them decides between two unfamiliar dishes with. Sections and items both carry an explicit position, because alphabetical is not an ordering anybody chose. Section names are unique case-insensitively where item names are deliberately not — two rows called "Soup" is a rotating special, two headings called "Drinks" is a mis-tap the guest sees as the same heading twice. A section may be deactivated, which hides the heading from the guest and leaves its items' own availability untouched. Every one of these is an append-only event like every other menu change. The mechanism is ADR-0014 and technical specification §7; the staging is `docs/MENU_AND_HANDHELD_PLAN.md`. **This is the first requirement in this document that came from a user rather than from its author.**

Deliberately still out of scope, and recorded so it is not mistaken for an oversight: allergen and dietary flags, because displaying "gluten free" without a mechanism that guarantees it is worse than saying nothing, and the same sentence about customization notes above is this project's position on the whole class; per-section serving windows, because "served until 11am" in a section description costs nothing and a scheduling model costs a milestone; and sub-sections, because a tree is a recursive query and a recursive renderer for a menu with eight headings on it.
...
- **Time:** everything stored `timestamptz` UTC; rendered in `RESTAURANT_TIME_ZONE` (default `America/New_York`) — **always, for every reader, wherever they are**. A restaurant is a physical place in one zone, so the viewer's own zone is deliberately irrelevant: a guest in New York reading the history of a meal they ate in Tokyo sees Tokyo times, and every screen in the building agrees to the minute. Because that convention is invisible until stated, a footer on every page shows the restaurant's current date and time, ticking to the second (technical specification §11.7). Whether times read `3:04 PM` or `15:04` is `RESTAURANT_CLOCK_FORMAT` (default `12-hour`), display only.
...
- **The program says what it is, and offers its own source.** Every page's footer names the running version and links to a page that offers the corresponding source under AGPL-3.0-only, at a URL the operator sets (`RESTAURANT_SOURCE_URL`, technical specification §11.9). Two things ride on this. Operationally, "which build is this box running?" must be answerable from the running instance rather than from whoever deployed it — so the version and the source revision are stamped into the build and reported on the page and in the OpenTelemetry resource. Legally, this project is published so that other people will run it, and §13 of the AGPL asks a **modified** version to offer its users the corresponding source; shipping the mechanism means a fork operator complies by changing one environment variable rather than by writing a page. There is deliberately no setting that turns the offer off.
- **Published software must be reportable-to.** A project that invites strangers to read its source is inviting the population that finds security defects, so it must offer them a way to say what they found that is not a public page. The channel is private by default, it is named in `SECURITY.md`, and it is the **one** exception to the no-outside-contributions rule — because refusing a feature costs the person who wanted it, who has the source and the freedom to build it, while refusing a report costs an operator's *guests*, who never chose this software and have no fork to run. Two things follow. The accepted-risks register (technical specification §17) is part of the offer rather than a disclaimer, so a reporter is not spending unpaid evenings re-deriving a decision that was argued and written down. And a document in this tree states **policy, never platform state**: what the project will do is true wherever it is read, whereas a claim about a hosting provider's settings page is unverifiable from inside the repository and was, in fact, wrong for as long as it existed (F-42).
- **A program published for strangers to run must constrain what a browser may do with the pages it serves.** Every response names its own content-security policy, refuses to be framed, refuses to have its content type guessed at, and refuses to carry a URL of this site into anybody else's logs (technical specification §11.11, ADR-0013). The reason this is a requirement rather than a deployment note is that the deployment is not one thing: this software is fronted by a local reverse proxy in development, by a tunnel provider in production, by an optional proxy on a staff LAN, and by nothing at all when somebody runs it to reproduce a defect — so a protection configured in front of it is a protection that is absent in most of the places it runs, and absent from every fork that chose differently. Two of this project's own rules meet here. The one about platform state (below) says a document must not assert a setting nothing in the repository can verify; a *control* that exists only as a setting on somebody's dashboard is that mistake with worse consequences. And the guest is the person this protects: a guest scans a code on a table, is handed a URL by a piece of paper, and has no way to evaluate what the page they land on is allowed to do.
- **The program must be operable on the screen §1 says it is used from.** §1 has said since rev 1 that guests order from their own phones. The staff who run the floor are holding the same phones: nobody carries a desktop computer with an ultrawide monitor to a restaurant, and an operator managing a table is standing at that table. So every surface is laid out for a handset **first** and widened where there is room, rather than the other way round — and the direction is the requirement, not a styling preference, because the inverted arrangement fails in the worst available direction: whatever is forgotten or unapplied lands on the layout that does not work on the screen the software is actually used from. Concretely: one breakpoint; a control a person presses is at least 44 pixels tall; a text field's own text is at least 16 pixels, because under that a phone browser zooms the page and does not zoom back; a row of records is a list of cards on a narrow screen, with each value saying what it is; and the way into a row is never a column on the far right of a table that scrolls sideways. Technical specification §11.12 is the mechanism. This is entered as **F-59** and for the reason revisions 3, 4 and 5 were entered rather than the reason a mechanism fix would be — nothing in this document previously asked for it, so it is new intent. It was reported by the first person outside this project to be shown the running application, which is worth recording: every gate was green, all fifteen end-to-end scenarios were passing, and the pages were unusable on the device they were written for.
...
License: **AGPL-3.0-only** (`LICENSE`). Governance: single-owner project, no outside contributions (`CONTRIBUTING.md`) — with **one exception, security reports** (`SECURITY.md`, §8); documentation changes are atomic — a behavior change lands with its requirement, specification, review-ledger, and ADR edits in one commit; ADRs are edited in place with a History line rather than duplicated.

---

## Revision history

- **Rev 6 — 2026-08-11.** One new bullet in §8 — the program must be operable on the screen §1 says it is used from — and the first substantive edit to §6.8 since rev 2, giving the menu sections, descriptions and explicit ordering. Both came from **one conversation with the first person outside this project to be shown the running application**: an enhancement request for the menu, and a report that the Manage button on the administration tables page sat off the right-hand edge of a phone screen. That is worth recording as its own kind of event. Every finding in the ledger up to F-58 was found by this project looking at itself — reading the tree, running a command, asking what a tag would make true — and F-59 was found in ninety seconds by somebody holding a phone, with all fifteen end-to-end scenarios green. The layout principle is entered as **F-59** on rev 3, 4 and 5's reasoning: nothing here previously asked for it, so it is new intent rather than a mechanism catching up. The menu edit takes no F-number, because an enhancement is not a defect and the ledger is for findings; its trail is ADR-0014, `docs/MENU_AND_HANDHELD_PLAN.md` and technical specification §19's M7. **And one correction:** this document's header read "Revision 4" from 2026-08-06 until today, above the rev 5 entry below, and cited the specification at v1.6 while it was at v1.14 — **F-58**, the third occurrence of a defect whose fix was made executable in Slice 23 against one filename. Nothing else moved: no role, no flow, no credential.
- **Rev 5 — 2026-08-06.** One new bullet in §8: a program published for strangers to run constrains what a browser may do with the pages it serves. Entered as **F-49**, and for the reason rev 3 and rev 4 were entered rather than the reason a mechanism fix would be — nothing in this document previously asked the program to say anything about framing, content sniffing, script sources or referrers, so this is new intent. The finding that produced it is not that the protection was missing: a partial one was already being emitted by the framework, on a subset of responses, carrying a single directive, and no document in this tree mentioned that it existed. Nothing else moved: no schema, no role, no flow, no surface.
- **Rev 4 — 2026-08-05.** One new bullet in §8: published software must be reportable-to, and a document states policy rather than platform state. Entered as **F-42** in the same review that produced F-39 and for the same reason — the moment before publishing is a distinct review — but one layer further out. F-39 found that the *program* could not say what it was; F-42 found that the *repository* offered a reader nowhere to say what they had found, while the only document addressing the question asserted a setting that was not set and never had been. §10's governance sentence gains the carve-out. Nothing else moved: no schema, no role, no flow, no surface.
- **Rev 3 — 2026-08-04.** One new bullet in §8: the running program names its own version and offers its corresponding source. Entered as **F-39** ahead of the first tagged release, on the same reasoning as F-35 and F-37 — a capability the surrounding documents assumed and no milestone had claimed. `CONTRIBUTING.md` has said since rev 1 that a fork "owes its users the same", and nothing in the application made that dischargeable; the offer now ships as a mechanism (`RESTAURANT_SOURCE_URL` → `/source`, linked from every footer) rather than as a page each operator must remember to write. Nothing else in this document moved: no schema, no role, no flow.
