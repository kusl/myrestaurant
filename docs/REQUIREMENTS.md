...
**Revision 4 — 2026-08-05.** Revision 2 folded in the owner's rulings on documentation-review findings F-01 through F-33 (see `docs/DOCUMENTATION_REVIEW.md` for the ledger and `docs/adr/` for the decision records). Revision 3 added one cross-cutting principle — the running program must offer its own source (§8, F-39). Revision 4 adds its counterpart: published software must be reportable-to (§8, F-42). Earlier revisions are preserved in git history. The companion `docs/TECHNICAL_SPECIFICATION.md` v1.6 is the normative implementation contract; where this document states intent and that document states mechanism, the mechanism governs.
