...
**Revision 3 — 2026-08-04.** Revision 2 folded in the owner's rulings on documentation-review findings F-01 through F-33 (see `docs/DOCUMENTATION_REVIEW.md` for the ledger and `docs/adr/` for the decision records). Revision 3 adds one cross-cutting principle — the running program must offer its own source (§8, F-39) — ahead of the first tagged release. Earlier revisions are preserved in git history. The companion `docs/TECHNICAL_SPECIFICATION.md` v1.3 is the normative implementation contract; where this document states intent and that document states mechanism, the mechanism governs.
