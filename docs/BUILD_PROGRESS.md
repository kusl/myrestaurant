...
**Sensitivity.** Every new assertion was proven able to fail against a planted defect. `TheSchemaRefusesEveryForbiddenShapeOfAnEventRow` was run in four mutations of `0009` — vocabulary widened to admit `loved`, the payload biconditional weakened to a single implication, the blank guard dropped, and the cap constraint renamed — and each mutation is reported by the constraint name the assertion names, which is what makes the four constraints separate rather than one conjunction. `ABodyOverTheDeclaredCapIsRefusedAndTheCapIsReadFromTheConstraint` was run with `ReadDeclaredBodyCapAsync` returning a hardcoded 1000 against a migration whose cap was moved to 900, and it fails at the at-cap submission rather than at the over-cap one. `ResubmittingTheSameBodyWritesNothing` was run against a writer with the `NoChange` branch removed and against one comparing untrimmed bodies; both report a second event. `TheStaffReadNamesTheAuthorAndFallsBackToTheUsername` was run against a read using `display_name` bare, which returns two blanks for `ben`.
