# Build progress — archive, through M6 Slice 39

**This file is the first part of one log, and the log's name is `BUILD_PROGRESS`.** It holds every slice
from M1 through **M6 Slice 39**; `docs/BUILD_PROGRESS.md` holds M6 Slice 40 onward and is the file a
slice is appended to. A citation of the form *BUILD_PROGRESS M6 Slice N* — which is how every row in
`docs/DOCUMENTATION_REVIEW.md` and in the specification's Appendix A refers to this log — resolves to
whichever of the two files holds slice N. There is exactly one rule for which: slices 1 through 39 are
here, 40 onward are there.

**Why it was split, and what that costs (F-96).** `export.sh` writes a context dump for an LLM session,
and this log had grown to 829 KiB — 13% of a 6.08 MiB dump, almost all of it slices closed months ago.
`export.sh` therefore excludes `docs/progress/` from the dump by path, exactly as it already excludes
`docs/llm/`. The cost is stated rather than discovered: **a session working from `dump.txt` cannot see
this file**, so nothing in a session may reconstruct it, and the retained document must never be
delivered as though it were the whole log. `ContextDumpExclusionContractTests` holds the tree to the half
of that which is checkable — that every archived document is linked by path from a document the dump does
contain, so history that leaves the dump leaves a pointer behind.

**Nothing here was edited in the split.** The bytes below are the bytes that were above `# M6 Slice 40`,
with the old `# Build progress` heading replaced by this one. Git holds the proof.
